functions {
  real customProb(int n, int p, int r, real q, real tow02, real tow2, real a, real b, matrix Y, matrix X, matrix L, matrix S_U, matrix S_V, vector d, matrix S, real sigma2){
    matrix[n,p] Z;
    real out;
    
    Z = Y - (X*(L + S));
    out = - (((n*p*0.5) + a + 1)*log(sigma2)) - ((1/(2*sigma2))*trace(crossprod(Z))) - ((0.5)*trace(S_U + S_V)) - ((0.5)*(d'*d)) - (b/sigma2) + sum(log((q/sqrt(tow02))*exp((-1/(2*tow02))*(S .* S)) + ((1-q)/sqrt(tow2))*exp((-1/(2*tow2))*(S .* S))));
...
transformed parameters {
  // We calculate the orthogonal matrix U and V from X_1 and X_2 respectively using the polar decomp.
  matrix[n,r] U;
  matrix[p,r] V;
  matrix[r,r] S_U;
  matrix[r,r] S_V;
  matrix[n,p] L;
  {
    ...
    U = (X_1)*diag_post_multiply(evec1, eval_trans1)*evec1';        // Use diag_post_multiply(evec, eval_trans), which is more efficient than using diag_matrix(). Similarly true for pre-multiplication as well.
